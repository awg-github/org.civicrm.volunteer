  ...
  <table>
    ...
    {foreach from=$assignments.values item=assignment}
      <tr>
        ...
        <td>
          {if $assignment.email}
            {assign var='emailUrlParams' value="reset=1&cid=`$assignment.contact_id`&nid=`$assignment.need_id`"}
            <a class="button" href="{crmURL p='civicrm/vol/email' q=$emailUrlParams}">
